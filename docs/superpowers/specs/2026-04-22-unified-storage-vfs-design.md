...
统一虚拟目录树中，同一父目录下所有对用户可见的节点名称必须唯一。
...
1. **挂载点参与占名**
   - 若已存在挂载 `/docs/team`，则在 `/docs` 下，`team` 这个名称视为已被占用
2. **Yunxia 内部所有会引入新名称的操作都必须做冲突校验**
   - 创建挂载
   - 新建目录
   - 上传新文件
   - 重命名
   ...
   - 复制
3. **若检测到同级重名，操作直接失败**
   - 返回统一冲突错误，而不是尝试覆盖、猜测或自动改名
4. **对外部旁路写入导致的冲突，不承诺物理阻止**
   - 例如管理员绕过 Yunxia 直接在本地磁盘创建了与挂载点同名的目录
   - 第一阶段处理策略为：
     - 挂载点优先显示
     - 记录告警日志
     - 后台提示管理员修复
...
所有会引入新名称的文件操作，都必须在目标父目录执行“同父目录名称唯一”校验。
...
1. `mount_path` 全局唯一
2. `mount_path` 的父目录语义成立
3. `mount_path` 在其父目录中不会与现有可见节点同名

若冲突，则拒绝提交，并返回 `MOUNT_PATH_CONFLICT` 或 `NAME_CONFLICT`。
...
- **Yunxia 内部操作层面，默认禁止同父目录重名**
- 挂载点也参与占名
- 创建挂载、新建目录、上传、重命名、移动、复制时都必须先做冲突校验
- 若是外部旁路写入导致的冲突：
  - 挂载优先显示
  - 记录告警日志
  - 提示管理员处理
