...
|---|---|---|---|---|---|---|---|
| 阻塞 | 2026-05-05 | 存储源/PikPak | 存储源管理页、文件页/VFS、上传弹窗、离线下载页、RSS/追番目标目录、WebDAV 配置 | P1 | `/api/v1/sources*`、`/api/v2/fs*`、`/api/v1/upload*`、`/api/v1/tasks`、`/dav/{slug}` | PikPak 源创建/编辑/secret 掩码，PikPak VFS 写操作/删除回收站文案，上传 server/direct 分支，`pikpak_native` 任务展示与取消，非 local WebDAV 暴露；captcha 验证链接入口和 `/files?path=` 深链已前端修复待回归，PikPak 正向链路仍阻塞 | [详情](#test-handoff-2026-05-05-pikpak-storage-adaptation) |
